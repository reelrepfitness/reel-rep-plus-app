...
export default function HomeScreen() {
  ...
  const handleCupToggle = (cupIndex: number) => {
    if (cupIndex < waterGlasses) {
      updateWater(cupIndex);
    } else {
      updateWater(cupIndex + 1);
    }
  };
            ...
            <View style={styles.waterCupsRow}>
              {Array.from({ length: 6 }, (_, i) => (
                <TouchableOpacity
                  ...
                  onPress={() => handleCupToggle(i)}
                  activeOpacity={0.7}
                >
                  <CupSoda
                    size={40}
                    color={i < waterGlasses ? "#3FCDD1" : "#9CA3AF"}
                    strokeWidth={2}
                    fill={i < waterGlasses ? "#3FCDD1" : "transparent"}
                  />
                </TouchableOpacity>
              ))}
            </View>
            <View style={styles.waterCupsRow}>
              {Array.from({ length: 6 }, (_, i) => (
                <TouchableOpacity
                  key={i + 6}
                  style={styles.waterCupButton}
                  onPress={() => handleCupToggle(i + 6)}
                  activeOpacity={0.7}
                >
                  <CupSoda
                    size={40}
                    color={(i + 6) < waterGlasses ? "#3FCDD1" : "#9CA3AF"}
                    strokeWidth={2}
                    fill={(i + 6) < waterGlasses ? "#3FCDD1" : "transparent"}
                  />
                </TouchableOpacity>
              ))}
            </View>
