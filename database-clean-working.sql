-- ============================================
-- CLEAN WORKING DATABASE - NO INFINITE RECURSION
-- ============================================
...
-- Profiles: Only use auth.uid() - NO SUBQUERIES TO PROFILES
...
-- Recipes: Users own their recipes
CREATE POLICY "Users can manage own recipes" ON recipes
  FOR ALL USING (auth.uid() = user_id);

-- Daily Logs: Users own their logs
...
-- Daily Items: Users own their items
...
-- Admin Audit Logs: No policies (admins disabled for now)
...
BEGIN
  ...
  RAISE NOTICE '🔐 RLS enabled (NO infinite recursion)';
  RAISE NOTICE '📝 Target templates: 13';
  RAISE NOTICE '🍽️  Restaurants: 107';
  RAISE NOTICE '';
  RAISE NOTICE 'Next step: Import your food_bank CSV data';
END $$;
